...
LANHostname=`getent hosts $(ip a |awk '/: e/{gsub(/:/,"");print $2}') | awk '{print $2}'`
...
cat <<EOF
Router Address  : $(echo $LANAddress | awk -F "." '{print $1 "." $2 "." $3 ".1"}')
Router Hostname : $(getent hosts $(echo $LANAddress | awk -F "." '{print $1 "." $2 "." $3 ".1"}') | awk '{print $2}')
EOF
...
cat <<EOF
Network Number. : $(echo $LANAddress | awk -F "." '{print $1 "." $2 "." $3 ".0"}')
Network Name    : $(getent networks $(echo $LANAddress | awk -F "." '{print $1 "." $2 "." $3 ".0"}') | awk '{print $1}')
EOF
